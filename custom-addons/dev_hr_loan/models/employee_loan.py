...
class employee_loan(models.Model):
    ...
    @api.constrains('loan_amount', 'installment_lines')
    def compute_total_installment_amt(self):
        for rec in self:
            if rec.installment_lines:
                sum_installment_amt = 0
                for record in rec.installment_lines:
                    sum_installment_amt += record.installment_amt
                if sum_installment_amt != rec.loan_amount:
                    raise ValidationError(
                        "Sum of installment amount should be equal to %s please check." % rec.loan_amount)
    ...
    def is_department_manager(self):
        for loan in self:
            if loan.manager_id:
                ...
                if loan.employee_id.user_id.id == self.env.user.id:
                    loan.is_dm = True
                else:
                    loan.is_dm = False
