...
class Users(models.Model):
    ...
    menu_ids = fields.Many2many('ir.ui.menu', 'user_menu_rel', 'user_id', 'menu_id',
...
class ResGroups(models.Model):
    ...
    menu_ids = fields.Many2many(
        'ir.ui.menu', 'group_menu_rel', 'group_id', 'menu_id', string='Menu To Hide')
...
class IrActionsReport(models.Model):
    ...
    hide_user_ids = fields.Many2many(
        'res.users', 'user_report_rel', 'report_id', 'user_id', string='Hide From Users')
    hide_group_ids = fields.Many2many(
        'res.groups', 'group_report_rel', 'report_id', 'group_id', string='Hide From Groups')
...
class IrUiMenu(models.Model):
    ...
    hide_group_ids = fields.Many2many(
        'res.groups', 'group_menu_rel', 'menu_id', 'group_id', string='Hide From Groups')
    hide_user_ids = fields.Many2many(
        'res.users', 'user_menu_rel', 'menu_id', 'user_id', string='Hide From Users')
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        if self.env.user == self.env.ref('base.user_root'):
            ...
            if menus:
                ...
                menu_ids2 = [
                    menu for group in self.env.user.groups_id for menu in group.menu_ids]
...
class FieldConfiguration(models.Model):
    ...
    def _return_field_domain(self):
        return [('model_id.name', 'ilike', self.model)]

    model_id = fields.Many2one(
        'ir.model', string='Model', ondelete='cascade')
    model = fields.Char(string='')
    field_id = fields.Many2one('ir.model.fields', string='Field', required=True,
                               ondelete='cascade', domain=_return_field_domain)
    field_name = fields.Char(related='field_id.name',
                             string='Technical Name', readonly=True)
    group_ids = fields.Many2many('res.groups', 'field_config_group_rel',
                                 'group_id', 'field_config_id', required=True, string='Groups')
