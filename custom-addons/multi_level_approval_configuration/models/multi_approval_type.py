  ...
  rec['{f_name}'] = rec.env['multi.approval.type'].compute_need_approval(rec)
	...
	def create_view(self, f_name, f_name1, f_name2):
		...
		wiz_act = self.env.ref(
			...
			False)
		wiz_app_act = self.env.ref(
			'multi_level_approval_configuration.action_model_approve', False)
		...
		if not wiz_act or not wiz_view_act or not wiz_rework_act or not wiz_app_act:
		...
		btn_app_node = E.button(
			{'approval_btn': '1'},
			name = str(wiz_app_act.id),
			type="action",
			string="Approve",
			groups="multi_level_approval.group_approval_user",
			attrs=str({
				'invisible': ['|', '|', (f_name2, '=', False), (f_name, '=', False), (f_name1, '!=', False)]
			})
		)
	...
	def action_approve(self):
		...
		multi_model = self.env['multi.approval'].search([
			('origin_ref', '=', origin_ref)
			], limit=1)

		ret_act = None
		for rec in multi_model:
			if not rec.is_pic:
			...
			for line in lines:
				...
				if not line or line.state != 'Waiting for Approval':
					...
					raise ValidationError(_('Something goes wrong!'))
					# return False
				...
				_logger.error(f"line sequence {line.sequence}")
				_logger.error(f"other lines {other_lines}")     
				if not other_lines:
					_logger.error("entered other lines")
	...
	def send_email(self, rec, line):
		...
		rec.sudo().message_post(body=msg, message_type='notification', author_id=self.env.user.partner_id.id, notification_ids=notification_ids)
