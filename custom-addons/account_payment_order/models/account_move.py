...
class AccountMove(models.Model):
    ...
    def _check_ref(self):
        if self.ref:
            valid_ref= self.env['account.move'].search([('ref', '=', self.ref),('partner_id','=',self.partner_id.id),('move_type','=','in_invoice') ])
            if len(valid_ref) >= 2:
                raise ValidationError("The Reference number already exists for this Partner!")
