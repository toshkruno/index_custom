...
class AccountMove(models.Model):
    ...
    @api.depends('line_ids.withholding_vat', 'line_ids.withholding_tax_id')
...
class AccountMoveLine(models.Model):
    ...
    withholding_vat = fields.Boolean(string="Withholding")
    ...
    def onchange_product_id(self):
        if self.product_id.apply_withholding:
            self.withholding_vat = True
        else:
            self.withholding_vat = False
    ...
    @api.depends('quantity', 'price_unit', 'withholding_vat', 'withholding_tax_id')
    def _compute_withholding_subtotal(self):
        for rec in self:
            if rec.withholding_vat and rec.withholding_tax_id:
